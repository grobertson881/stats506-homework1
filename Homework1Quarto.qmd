...
3. The abalone with the most rings has the following weights: Whole weight = 1.8075; Shucked weight = 0.7055; Viscera weight = 0.3215; Shell weight = 0.475. 4. 6.512% of abalones have a viscera weight larger than their shell weight.
...
5. For food expenditures, remove blank or negative values 6. Only look at people who are primarily making food at home (< 10 times dining out)
...
for (i in 1:(length(inputValues))) {
  ...
  if (currentSeqLength > maxLength) {
    ...
    maxInput <- inputValues[i]
  }
  
  if (currentSeqLength < minLength) { 
    minLength <- currentSeqLength 
    minInput <- inputValues[i]
  }
}

print(paste("Max length: ", maxLength, " with input: ", maxInput))
print(paste("Min length: ", minLength, " with input: ", minInput))

```

*ANSWER*

The longest Collatz sequence is found with the input of 327. This sequence has a length of 144. 
The shortest Collatz sequence is found with the input of 128. This sequence has a length of 8.
